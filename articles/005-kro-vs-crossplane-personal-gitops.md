---
title: "Kro と Crossplane、個人 GitOps ではどちらを選ぶか"
emoji: "🧩"
type: "tech"
topics: ["kubernetes", "kro", "crossplane", "gitops", "kustomize"]
published: false
---

## はじめに

Kubernetes マニフェストを GitOps で管理していると、「複合リソースの取り扱い」と「クラウドリソースの宣言的管理」で詰まる場面がある。これらを解決する OSS として **Kro (Kube Resource Orchestrator)** と **Crossplane** がある。両者は重なる領域があるが思想が異なる。個人 GitOps の文脈で選び方を整理する。

## 個人 GitOps が直面する問題

自宅 k8s で 20+ アプリを ArgoCD + Kustomize で deploy していると、以下の問題に直面する:
...
**[Kro](https://kro.run/)** は kubernetes-sigs プロジェクトで、2024 年後半に公開された比較的新しい OSS。最新 v0.9.2 (2026-05 時点)。
...
私の判断: **Kro を採用、Crossplane は保留**

理由:

1. **Pi の RAM 制約**: Crossplane core + Provider AWS で 1GB+ 食う。Pi 4B 4GB では他 Pod の余裕が無くなる
2. **AWS 中心の構成**: GCP/Azure を使う予定がない。Crossplane の multi-cloud は overkill
3. **学習コスト**: Composition の設計は時間がかかる。RGD は K8s YAML の延長で書ける
4. **ACK との相性**: AWS リソース管理は ACK Controllers (IAM, S3) + Kro RGD でカバー可能

ただし以下の場合は Crossplane:
...
- **Kro**: 軽量、K8s ネイティブ、個人 Pi 〜 中小規模に最適
- **Crossplane**: マルチクラウド、大規模、エコシステム成熟
- **個人 GitOps + EKS Hybrid Nodes** の文脈では **Kro 一択** に近い
