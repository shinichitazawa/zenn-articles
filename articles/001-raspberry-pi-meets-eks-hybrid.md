...
似た選択肢として EKS Anywhere、EKS on Outposts があるが、個人 Pi 利用では選択肢にならない。詳細は次回の比較編で書くが要点だけ:

- **EKS Anywhere**: 顧客 DC で control plane も含めて自前運用、AWS Region 連携はオプション。エンタープライズライセンス
- **EKS on Outposts**: AWS 所有の 42U ラックを顧客 DC に置く。物理サイズ・電力・契約 (3 年コミット数万ドル/月) で個人不可
- **EKS Hybrid Nodes**: 顧客 HW を EKS の一員に。月額 vCPU 時間単価 + VPN 料金、個人 Pi で現実的
...
EKS Anywhere / Outposts / Hybrid Nodes / K3s+Tailscale の 4 つを比較して、「個人 Pi では Hybrid Nodes 一択」になる理由を整理する。
