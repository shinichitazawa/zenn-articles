...
本記事は、セルフホストの n8n(v2.33.3)で AI エージェントを使った業務ワークフローを8本作り、そのすべてを実際に実行して検証する過程で詰まった点の記録です。**動いた・動かなかったの実測が中心**で、原因が公式ドキュメントで裏付けられるものはその出典を、記載がないものは実測である旨を明示しています。
