...
「Spot はグローバル枠の対象外では」という説を実測で潰しました。在庫のある us-central1-a で Spot 作成を試すと:
...
結論はシンプルで、**「毎日回す確信がつくまでは秒課金・時間課金で借りる」が数字上も圧倒的**です。特に検証フェーズ(数十時間)なら、DOK の 57.6 円/時は総額数千円で終わります。
...
3. **k8s クラスタに GPU ノードとして編入したい** → GCP/AWS の Spot VM 一択(DOK と Colab はジョブ/ノートブック型のため不可)。ただし**クォータ申請のリードタイムを工程に織り込む**こと。GCP は `GPUS_ALL_REGIONS`(Spot にも適用)、AWS は G/VT インスタンスの Spot vCPU 枠
4. **購入** → 稼働時間の実績が出てから。買うなら現時点の妙味は中古 RTX 3090 24GB(¥12〜15万、マイニング酷使個体を避けて保証付き店頭で)か、省電力の RTX 5060 Ti 16GB。RTX 5090 は BTO 一式が単体より安い相場の間は BTO で
