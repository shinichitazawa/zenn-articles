---
title: "OSS LLM 4 つを LiteLLM + EKS + KEDA で A/B 比較する"
emoji: "⚖️"
type: "tech"
...
- **Caller は無改修**。`model="ab-router"` に変えるだけで 4 候補へ振り分けられます（`simple-shuffle` は重み付きランダムです）
...
| モデル | サイズ | token/s |
|---|---|---|
| sarashina2.2-3b-instruct Q4_K_M | 3B | 40-60 |
...
[LiteLLM](https://docs.litellm.ai/docs/proxy/reliability) は OpenAI 互換 proxy で、同一 `model_name` を複数 endpoint で定義すると `routing_strategy` に基づいて振り分けます。`simple-shuffle` は round-robin ではなく重み付きランダムで、[公式](https://docs.litellm.ai/docs/routing)は rpm/tpm に基づいて重み付けし、未指定ならランダムに選ぶと説明しています（2026-08 時点で取得）。同一 prompt を 4 候補へ均等に配りたい場合は別の方式が要ります。

```yaml:litellm-config.yaml
model_list:
  - model_name: ab-router
    litellm_params:
      model: openai/sarashina2.2-3b-instruct-v0.1
      api_base: http://model-sarashina.ai-platform.svc.cluster.local
      api_key: dummy
    model_info: {ab_arm: "sarashina"}
  - model_name: ab-router
    litellm_params:
      model: openai/plamo-2-8b
...
Worker pod の権限は Secrets Manager の GetSecretValue 2 つだけにします。
