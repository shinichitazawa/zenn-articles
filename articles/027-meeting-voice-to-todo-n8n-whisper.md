...
デプロイは素の Kubernetes manifest で、構成は次のとおりです。

```mermaid
flowchart TB
  subgraph cluster[k3s クラスタ内で完結する。Ingress なし]
    N8N[n8n] -->|HTTP で音声を投入| WH[whisper-asr-webservice<br/>faster-whisper small int8]
    WH --> PVC[(モデルキャッシュ PVC<br/>初回のみ約 462MB 取得)]
  end
```

要点は次の3つです。

- **モデルキャッシュを PVC(PersistentVolumeClaim。Pod が消えても残る永続ストレージの要求)に置く**。small モデルは初回に約 462MB をダウンロードするため(実測)、Pod 再起動のたびに落とし直さないよう永続化する
...
| 台本に仕込んだ要素 | 例 | 何を検証するか |
|---|---|---|
| 品番と数量 | 「A001 を三十個発注」 | 型番・数値が文字起こしと正規化で保存されるか |
| 金額 2 種 | 「税抜き二十八万円」「合計三十二万円」 | 別議題の金額を混同しないか |
| 相対期限 3 種 | 「今週金曜までに」「来週水曜までに」「明日までに」 | コードでの絶対日付化が正しいか |
| 業務用語 | 見積、備品、稟議、納期、五営業日 | 同音異義の誤変換と正規化の効果 |
| 次回日程 | 「八月二十一日の十時から」 | ToDo と次回会議を区別できるか |
...
Polly 音声を通した結果です(実測)。**数値・金額・日付・人名はすべて正しく残りました**。A001、12個、30個、28万円、32万円、2台、8月21日の10時、田中さん、佐藤さん — 数値や人名が誤ると業務では使えないため、この結果が採用の前提になりました。
...
「数値は保持され、専門用語だけが誤認識される」という傾向なので、**後段に修正の工程を置けば成立する**と判断できます。これが次の正規化ステップの根拠です。

## 正規化(AI が直し、コードで検証)

誤認識の修正は AI(Bedrock 上のモデル)に任せますが、指示に制約を設けます。
...
違反があればフラグを立て、**正規化後ではなく原文を後段に渡します**。誤った正規化結果を入力にするより、誤字混じりの原文の方が影響が小さいためです。

```mermaid
flowchart TB
  RAW[文字起こし原文] --> AI[AI が誤変換を修正<br/>数値・金額・人名・型番は変更禁止]
  AI --> CHK[コードで照合<br/>数値の残存・人名の残存・文字数±20%]
  CHK -->|全条件を満たす| USE[正規化後テキストを抽出へ]
  CHK -->|1 つでも違反| FALL[原文を抽出へ + フラグ残置]
```

実測では「美品→備品」「品技→稟議」は直り、「三森→見積」は直りませんでした(モデルは Amazon Nova Lite。この品質差への対応は後述)。
...
この検証層は実測で効果がありました。初回の実行では AI が「購買申請書の作成」に本文に無い 30,000 円を付けましたが、裏取りで棄却されています。また 2 件の引用に「一致率 50%」のフラグが付き、AI の言い換えを検出しました。
...
応答ヘッダを 300 秒しか待たない、という制限です。10 分の会議なら処理は 40 分かかる想定なので、同期呼び出しで待つ設計は成立しません。
...
Data Table の[公式ドキュメント](https://docs.n8n.io/build/work-with-data/data-tables/)には、アクセス範囲が「同じプロジェクトのメンバー全員」であることと容量上限(既定 200 MiB)の記載はありますが、行ごとの変更履歴や表単位の権限管理といった機能は記載されていません(2026-09 時点)。ToDo 管理に使うなら、その部分は自前で設計する必要があります。
...
**ToDo 管理そのものは Jira などの専用ツールの方が適しています**。今回 Data Table で自作したもの(状態遷移の管理・変更履歴・リマインド)は、専用ツールなら最初から付いています。表単位の権限管理やダッシュボードも[公式ドキュメント](https://docs.n8n.io/build/work-with-data/data-tables/)に記載がありません(行の filter はあります。2026-09 時点)。
...
3. **処理時間が読めない同期 API は n8n から直接呼べない**。undici の headersTimeout(既定 300 秒)で切断される。受付 + ポーリングの非同期に分ける
