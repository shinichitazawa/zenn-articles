---
title: "Tailscale auth key の自動 rotation を Argo Events で組む"
emoji: "🔑"
type: "tech"
...
| OAuth client の scope 不足 / tag 未指定 | `403 Forbidden` で key 発行不可 | [OAuth client doc](https://tailscale.com/docs/features/oauth-clients) 通り、scope `auth_keys` 付与時は tag 指定が必須 (本構成では `tag:subnet-router`)。ACL の autoApprover で routes を同 tag に紐付けて整合させる |
...
- NATS JetStream Streams: https://docs.nats.io/learn/jetstream/your-first-stream
- NATS JetStream Consumers: https://docs.nats.io/learn/jetstream/pull-consumers
- NATS Subjects (wildcards / reserved prefix): https://docs.nats.io/concepts/subjects
- NATS JetStream API reference (`$JS.API.*`): https://docs.nats.io/reference/jetstream/api/
- NATS JetStream 有効化 (config / `-js` flag): https://docs.nats.io/reference/config/
- nats-server source (Go): https://github.com/nats-io/nats-server
- NATS Monitoring (`/jsz` endpoint): https://docs.nats.io/learn/monitoring/monitoring-endpoints
