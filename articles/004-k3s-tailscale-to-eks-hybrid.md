...
自宅 Raspberry Pi で K3s + Tailscale 構成を 1 年以上運用してきたが、EKS Hybrid Nodes に移行する。本記事は移行ステップと、設計上の判断と落とし穴を整理する。
...
2. **自宅ルータ S2S VPN 設定**: tunnel up + BGP 経路交換
...
2. **SSH 経路**: Pi に SSH するのに自宅 IP / DDNS より楽
...
→ **Private only** で開始 (VPN 必須)、kubectl は AWS bastion 経由 or 自宅から VPN 経由。
...
| remote_node_cidr | 192.168.10.0/24 (自宅 LAN) |
